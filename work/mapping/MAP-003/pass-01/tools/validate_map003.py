#!/usr/bin/env python3
"""MAP-003 Mapper self-check; separate independent Validation is still required."""
...
def run(mapfile: Path, sample_zip: Path, story_zip: Path) -> list[str]:
    ...
    events = data["events"]
    check("10 contiguous and exact inert anchors", len(events) == 11 and events[0] is None and
