...
Use a newly created Eryndra review project only. Never install into `SampleGenerated`, either ZIP, or an extracted reference tree.
...
```powershell
py install_into_eryndra_review.py "C:\full\path\to\Eryndra_MAP002_Review" --dry-run
py install_into_eryndra_review.py "C:\full\path\to\Eryndra_MAP002_Review"
```

The project must already contain registered maps 1 and 2, the accepted pre-transfer `Map001.json`, the accepted Mapping Pass 01 `Map002.json`, MAP-001's `Ancient_ThreeNote_Resonance` audio, and the listed stock placeholder sheets. The installer refuses incompatible or reference targets and creates recoverable backups before replacement.

## Test A — MAP-001 into Morning

1. Leave New Game start on MAP-001 `(14,10)`.
2. Start a New Game.
...
5. Confirm the exact Morning dialogue, one restrained dog sound, crooked-latch action, collision-safe kit movement and double-check motions.
...
3. The four distinct supper positions read clearly and the exact supper exchange plays.
